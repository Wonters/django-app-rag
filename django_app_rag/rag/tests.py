...
from django_app_rag.models import Source, Question, Collection, Document as DocumentModel, Answer
...
class TestRetriever:
    ...
    def test_document_chunks(self):
        ...
        for i, chunk in enumerate(chunks):
            ...
            assert hasattr(chunk, 'page_content'), f"Le chunk {i+1} doit avoir un attribut page_content"
            assert hasattr(chunk, 'metadata'), f"Le chunk {i+1} doit avoir un attribut metadata"
            assert isinstance(chunk.page_content, str), f"Le contenu du chunk {i+1} doit être une chaîne de caractères"
            assert isinstance(chunk.metadata, dict), f"Les métadonnées du chunk {i+1} doivent être un dictionnaire"
            
            # Vérifier que le contenu n'est pas vide
            assert len(chunk.page_content.strip()) > 0, f"Le contenu du chunk {i+1} ne doit pas être vide"
        ...
        if chunk_sizes_by_id:
            ...
            print(f"  Écart-type: {sum((x - avg_chunk_size) ** 2 for x in chunk_sizes_by_id) ** 0.5 / len(chunk_sizes_by_id):.0f} caractères")
        ...
        total_chunks_by_id = sum(len(chunk_list) for chunk_list in chunks_by_id.values())
        total_chunks_by_doc_id = sum(len(chunk_list) for chunk_list in chunks_by_doc_id.values())
        
        assert total_chunks_by_id == len(chunks), "Le nombre total de chunks par ID doit correspondre"
        assert total_chunks_by_doc_id == len(chunks), "Le nombre total de chunks par doc_id doit correspondre"
...
class TestTaskQA:
    ...
    def test_task_qa(self):
        ...
        for i, question in enumerate(source.questions.all()):
            ...
            print(f"🔍 Documents: {answer_instance.documents.values_list('uid', flat=True)}")
